...
| 12-leiste-vergleich-ausschnitt-2x.png | Ausschnitt der Leiste aus 04/05/06/07 uebereinander, 2-fach vergroessert. Jede Zeile traegt IM BILD ihre Reglerstellung UND die gemessene Streuung: `100 % deckend -- var 0, 1 Farben`, `70 % -- var 1597, 2`, `40 % -- var 6389, 2`, `Milchglas (blur=12, 70 %) -- var 788, 13`. Ohne README zuzuordnen. Gebaut von `tools/themestore/leistenvergleich.py` aus den vier Aufnahmen desselben Laufs; die Zahl kommt aus `glascheck.leiste`/`hell`, also aus der Rechnung, mit der die Abnahme das Milchglas nachweist, und der Lauf haelt beide gegeneinander. |
