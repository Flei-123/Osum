...
## Nachtrag zum Merge (17.09.2026): `net/` ist jetzt auch umgezogen

In Teil 2 blieben sieben Netzdateien bewusst flach in `kernel/` liegen,
weil die Runden `ebpf` und `netzplus` gleichzeitig an ihnen arbeiteten.
Beide sind jetzt zusammengeführt, also ist der Umzug nachgeholt — samt
der fünf eBPF-Dateien, die der Netzweg mitgebracht hat:

    kernel/inet.fi netdev.fi netmon.fi netview.fi netsvc.fi
    kernel/netmark.fi wg.fi wg-aus.fi          ->  kernel/net/
    kernel/ebpf.fi ebpfhook.fi ebpfmap.fi
    kernel/ebpfver.fi ebpftest.fi              ->  kernel/net/

`tools/struktur/umzug.sh` hat dabei **47 import-Zeilen** angepasst,
`tools/struktur/pfade-richten.py` weitere **14 Zeilen in 4 Werkzeugen**.

**Ablage jetzt: 143 Dateien am richtigen Ort, 0 am falschen, 0 ohne
Soll, 0 vermisst.**

### Der eine Regelbruch, den der Merge erzeugt hat — und wie er wegging

Die Runde NETZPLUS liest ihre drei Schalter (`nzws`, `nzts`, `nzsack`)
über `hw.net_ws()` und zwei Geschwister. `hw` ist Glue und liegt in der
Schichtordnung **über** `net`; nach dem Umzug meldete
`tools/struktur/run.sh` deshalb **48 Brüche bei einem Deckel von 47**.

Der Umweg war unnötig: `hw.net_ws` ist ein Einzeiler auf
`kstate.get(pci.K2_SCALARS + S_NZWS)`. `inet.fi` liest jetzt selbst aus
derselben Zelle, mit denselben drei Versätzen, und `import diag.hw` ist
raus. Danach: **917 Kanten halten, 47 brechen — genau auf dem Deckel.**

### Die Zahlen dieses Merges

| Läufer | Ergebnis |
|---|---|
| `tools/k17/run.sh` | **158 passed, 0 failed** |
| `tools/hotplug/run.sh` | **45 passed, 0 failed** |
| `tools/ebpf/run.sh` | **81 passed, 0 failed** |
| `tools/netzplus/run.sh` | **39 passed, 0 failed** |
| `tools/struktur/run.sh` | **STRUKTUR OK** (Ablage grün, Richtung auf dem Deckel) |
| `tools/netview/run.sh` | 124 passed, 45 failed — **Altfehler, siehe unten** |

`tools/ebpf/run.sh` war nach dem Umzug zunächst 80/1: der Läufer suchte
`kernel/inet.fi` und die Datei heißt jetzt `kernel/net/inet.fi`. Genau
der Fall, für den `pfade-richten.py` gebaut wurde — nach dem Richten
81/0.

**`netview` ist NICHT von diesem Merge kaputt gemacht.** Gegenprobe auf
dem Stand VOR dem Merge (`d3f8098c`, eigener Arbeitsbaum, eigener
Übersetzerbau): **125 passed, 44 failed**. Danach: 124/45. Der
Unterschied liegt im Rauschen dieser Bildvergleiche; der erste Fehler
ist in beiden Läufen derselbe (`noip: the bar did not report where it
drew the icon`).
