#!/usr/bin/env bash
# tools/speicher/bauen.sh -- Kernel, Programme und die ZWEI Plattenabbilder
...
#   inhalt.img  DIESELBE ANORDNUNG, ABER MIT INHALT (tools/speicher/baum.py),
...
PROGS="speicher du sh echo ls cat suchen"
...
# --------------------------------------------------------- gross.img (Zeit)
python3 tools/k15/gross.py "$OUT/gross" 4000 > "$OUT/gross.log" 2>&1 || exit 1
...
python3 tools/speicher/baum.py "$OUT/baum" 4000 "$BUDGET" \
...
open(sys.argv[1], "wb").write(open(
    "tools/k15/baum.py").read().split('THEME = b"""')[1]
    .split('"""')[0].encode())
EOF
