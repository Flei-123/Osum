...
# DIE GRUENDE STEHEN ZWEIMAL, und das ist Absicht: `kernel/drivers/gfx/vmode.fi`
...
for r in CW_OK CW_REGS CW_VRAM CW_LIMIT CW_DEPTH CW_UNSINN CW_NOCARD; do
    a=$(grep -aE "^const $r: u64 = " kernel/drivers/gfx/vmode.fi | sed 's/.*= //; s/ *\/\/.*//')
...
gleich "Grund 1 heisst hier wie dort 'die Register haben abgelehnt'" "1" \
    "$(grep -aE '^const CW_REGS' kernel/drivers/gfx/vmode.fi | sed 's/.*= //')"
gleich "Grund 2 heisst hier wie dort 'zu gross fuer den Bildspeicher'" "2" \
    "$(grep -aE '^const CW_VRAM' kernel/drivers/gfx/vmode.fi | sed 's/.*= //')"
gleich "Grund 3 heisst hier wie dort 'dieser Kernel kann es nicht abbilden'" "3" \
    "$(grep -aE '^const CW_LIMIT' kernel/drivers/gfx/vmode.fi | sed 's/.*= //')"
...
hat kernel/user/settings.fi "DS_CUSTOM" \
    "das Bedienfeld ruft wirklich den neuen Aufruf und baut keine Liste nach"
hat kernel/user/settings.fi "DG_CUSTNUM" \
...
if grep -aq '(1400 << 16)' kernel/drivers/gfx/vmode.fi; then
...
schau "die Textzeile steht bildpunktgenau im eigenen Modus" \
    text "$TMPD/eigen.ppm" kernel/drivers/gfx/font.fi 14 0 "OSUM K7 FRAMEBUFFER 01234"
...
schau "und die Textzeile steht bildpunktgenau da, als waere nichts gewesen" \
    text "$TMPD/schranken.ppm" kernel/drivers/gfx/font.fi 14 0 "OSUM K7 FRAMEBUFFER 01234"
