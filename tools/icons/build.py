...
def ids_schreiben(pfad, liste, echte, gross, quelle_gross, v):
    z = []
    # RUNDE MERGE-2: DIE LIZENZZEILE GEHOERT DEM ERZEUGER, NICHT DER
    # ERZEUGTEN DATEI. Der SPDX-Durchgang hat sie `lib/icons.fi` in den
    # Kopf geschrieben -- einer Datei, die dieses Programm bei jedem Lauf
    # neu schreibt und die `tools/icons/run.sh` Oktett fuer Oktett mit dem
    # Erzeugnis vergleicht. Ab da war die Datei nicht mehr reproduzierbar,
    # und die Zusage "lib/icons.fi is reproduced octet for octet" war rot,
    # bei einem Unterschied von genau EINER Zeile. Entweder verliert die
    # Datei ihre Lizenzangabe oder der Erzeuger schreibt sie mit. Er
    # schreibt sie mit.
    z.append("// SPDX-License-Identifier: GPL-2.0-only")
