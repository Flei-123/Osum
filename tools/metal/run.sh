...
# tools/metal/run.sh -- OSUM AUF FREMDEM BLECH.
#
# ==================================================================
...
#
# Aufruf:  bash tools/metal/run.sh
set -uo pipefail
cd "$(dirname "$0")/../.."
. tools/lib/qemu.sh
...
if python3 tools/metal/chipnames.py >"$TMPD/chipnames.txt" 2>&1; then
...
if python3 tools/metal/r8125regs.py >"$TMPD/r8125.txt" 2>&1; then
