...
"""tools/vault/orphans.py -- the PRODUCER side of the orphan interface.
...
reasons:
  ...
  1. `/bin/backup` needs the list to be produced by SOMETHING before the
     measurement in `tools/vault/run.sh` can mean anything. A test that
...
USAGE
    ...
    python3 tools/vault/orphans.py --root TREE [--source DIR ...] [-o FILE]
