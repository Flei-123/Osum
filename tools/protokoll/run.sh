...
# tools/protokoll/run.sh -- RUNDE PROTOKOLL: DIE FUENF MESSUNGEN.
#
...
#       WIRKLICH GELESEN: `tools/protokoll/schirmtext.py` macht aus dem
...
if [ -s "$TMPD/panik.ppm" ]; then
    ...
    python3 tools/protokoll/schirmtext.py "$TMPD/panik.ppm" kernel/font.fi \
