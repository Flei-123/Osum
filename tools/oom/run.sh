...
echo
echo "== 2. ein Fresser: ehrliche Absage statt Freeze =="
run eins 128 memhog; RC=$?
F="$TMPD/eins.txt"
...
# Ist sie kleiner, hat der Notfall Speicher verloren -- genau der
# Fehler, den `do_brk` vor dieser Runde jedes Mal gemacht hat.
...
echo
echo "== 3. die Reserve greift, und sie ist gezaehlt =="
RES=$(zahl "$F" 'speicherdruck: res=')
HITS=$(zahl "$F" 'speicherdruck: hits=')
num "die Reserve steht" "$RES" ge 64
num "sie hat wirklich abgewiesen (hits)" "$HITS" ge 1
has "$F" 'oom: kill ' "der Kern sagt, wen es getroffen hat"
num "genau ein Prozess beendet" "$(zahl "$F" 'speicherdruck: kills=')" ge 1

# DIE GEGENPROBE. Ohne Reserve verhaelt sich der Kern wie vorher.
# Sie muss einen SICHTBAREN Unterschied machen, sonst waere die
# Reserve eine Behauptung: der Fresser kommt weiter, weil ihm die
# 512 Rahmen nicht mehr vorenthalten werden.
run ohne 128 "memhog noreserve"
G="$TMPD/ohne.txt"
