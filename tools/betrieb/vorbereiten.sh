...
# tools/betrieb/vorbereiten.sh -- alles, was VOR den Messungen entsteht:
# Pakete, Zertifikate, der Schluesselbund, vier Auslieferungen, das
# Abbild und eine Platte, auf der Fassung 1 laeuft.
#
#   bash tools/betrieb/vorbereiten.sh [ausgabeverzeichnis]
#
# Das dauert (Bau plus Installation in QEMU) und wird deshalb einmal
# gemacht; `tools/betrieb/run.sh` benutzt danach `$OUT/basis.img`.
set -uo pipefail
cd "$(dirname "$0")/../.."
ROOT=$(pwd)
...
# QEMUs Benutzernetz der Wirt, und dort laeuft `tools/betrieb/dnsdienst.py`.
