...
# tools/metal2/run.sh -- DIE ABNAHME DER RUNDE BLECH2.
#
#   bash tools/metal2/run.sh [--kern <osum.mb> --root <root.img>] \
...
#
#   A. QMP-MAUS. tools/metal2/maus.py schickt 60 s lang mit 1000 Hz
...
bash tools/metal2/lauf.sh "$TMPD/A" "$KERN" "$ROOT" "$SEK" "$HZ" "" \
    klickfeld=1800,700,3300,1250 klick_ms=2000 tasten=20 > "$TMPD/A.out" 2>&1 &
bash tools/metal2/lauf.sh "$TMPD/B" "$KERN" "$ROOT" "$SEK" 1 "mausflut=1000" \
    klick_ms=9999999 heim=0 > "$TMPD/B.out" 2>&1 &
bash tools/metal2/lauf.sh "$TMPD/C" "$KERN" "$ROOT" "$SEK" 1 "mausflut=4000" \
