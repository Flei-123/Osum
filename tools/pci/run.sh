...
for s in 0 1; do
    ...
    undef=$(nm -u "$TMPD/k$s.o" 2>/dev/null | awk '{print $NF}' | sed '/^$/d' | grep -vE '^(osum_panic|kdata|osym_tab)$')
    [ -z "$undef" ] && ok "k$s.o: no undefined name other than osum_panic, kdata und osym_tab (RUNDE PROTOKOLL: die Symboltabelle, schwach in boot.s)" \
