...
PROGS="sh ls cat echo date imgtest"
...
LOG="$TMPD/lauf.txt"
start=$(date +%s%3N)
timeout 1500 $QEMU_X86 -kernel "$TMPD/k.mb" -m 384 \
...
while read -r name w h z; do
    ...
    dz="${DIFFZEILEN[$d]:-}"
    d=$((d+1))
    if [ -z "$zeile" ] || [ -z "$dz" ]; then
        bad "$name: der Lauf hat dazu nichts gemeldet"
        continue
    fi
...
# =====================================================================
# 9. DIE PROBE AUF DEN UEBERSETZER
#
# Der Dreiecksfilter dieser Runde hat zuerst falsch gerechnet, und der
# Fehler lag nicht im Filter: `firnc` (a751b3db) legt drei ueber die
# Schleifenkante rotierende Veraenderliche auf zwei Stapelplaetze und
# verliert eine Kopie. Der Umweg steht in `imgjpeg.h2v2`; hier wird
