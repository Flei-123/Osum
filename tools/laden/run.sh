...
# tools/laden/run.sh -- DIE ABNAHME DER RUNDE LADEN.
#
#   bash tools/laden/run.sh [arbeitsverzeichnis]
#
...
bash tools/laden/abbild.sh "$OUT" > "$OUT/abbild.log" 2>&1 \
    && ok "Abbild gebaut" || { bad "abbild.sh"; tail -10 "$OUT/abbild.log"; exit 1; }
cp -f "$OUT/platte/disk.img" "$OUT/text.img"
LADEN_PLATTE="$OUT/text.img" bash tools/laden/lauf.sh a1 \
...
LADEN_PLATTE="$OUT/text.img" bash tools/laden/lauf.sh a2 \
...
python3 tools/laden/tippen.py "explorer" --warte 25 >> "$OUT/k-start.txt"
LADEN_WARTE=15 LADEN_KILL=ja bash tools/laden/lauf.sh a3 \
...
LADEN_PLATTE="$OUT/boese.img" bash tools/laden/lauf.sh a4 \
