...
#   2. DER ZEICHENSATZ. `kernel/drivers/gfx/font.fi` gegen die Rust-Vorlage, aus der
...
if [ -f "$ORIENTOS/kernel/src/drivers/font.rs" ]; then
    schau "font.fi ist die Vorlage, Oktett fuer Oktett" \
        font kernel/drivers/gfx/font.fi "$ORIENTOS/kernel/src/drivers/font.rs"
else
    schau "font.fi hat 95 Glyphen zu 16 Oktetten" font kernel/drivers/gfx/font.fi
...
w_fb=$(grep -E '^const WIN_SLOTS|^const WIN_FIRST|^const WIN_VIRT|^const HUGE_SIZE' kernel/drivers/gfx/fb.fi | sed 's/ *\/\/.*//' | sort)
...
k2=$(grep -E '^const K2_SCALARS' kernel/drivers/bus/pci.fi | grep -oE '0x[0-9A-Fa-f]+')
sw=$(grep -E '^const S_WIN' kernel/arch/x86_64/apic.fi | grep -oE '0x[0-9A-Fa-f]+')
fbl=$(grep -E '^const WIN_LIST' kernel/drivers/gfx/fb.fi | sed 's/.*= *//; s/ *\/\/.*//')
if [ "$fbl" = "$k2 + $sw" ]; then
...
schau "Zeile 14 bildpunktgenau: 'OSUM K7 FRAMEBUFFER 01234'" \
    text "$TMPD/pat.ppm" kernel/drivers/gfx/font.fi 14 0 "OSUM K7 FRAMEBUFFER 01234"
schau "Zeile 15 bildpunktgenau: 'abcdefghijklm ABCDEFGHIJK'" \
    text "$TMPD/pat.ppm" kernel/drivers/gfx/font.fi 15 0 "abcdefghijklm ABCDEFGHIJK"
...
schau_nicht "die Textzeile steht NICHT da" \
    text "$TMPD/keine.ppm" kernel/drivers/gfx/font.fi 14 0 "OSUM K7 FRAMEBUFFER 01234"
...
schau "der ganze Bildschirm gegen den seriellen Mitschnitt" \
    konsole "$TMPD/kon.ppm" kernel/drivers/gfx/font.fi "$TMPD/kon.txt" \
...
schau_nicht "gegen den Mitschnitt des Pruefbild-Laufs geht es NICHT auf" \
    konsole "$TMPD/kon.ppm" kernel/drivers/gfx/font.fi "$TMPD/pat.txt" \
    "fb: console mirrored to screen" "fb: hold" 500
...
schau "und der Text ebenso, bildpunktgenau" \
    text "$TMPD/big.ppm" kernel/drivers/gfx/font.fi 14 0 "OSUM K7 FRAMEBUFFER 01234"
...
if [ "$gebaut" = 1 ]; then
    ...
    schau "der ganze Bildschirm gegen den Mitschnitt der Shell" \
        konsole "$TMPD/sh.ppm" kernel/drivers/gfx/font.fi "$TMPD/sh.txt" \
        "fb: console mirrored to screen" "fb: hold" 500
    ...
    schau "die Zeile der Shell steht bildpunktgenau auf dem Schirm" \
        finde "$TMPD/sh.ppm" kernel/drivers/gfx/font.fi "$TMPD/sh.txt" \
