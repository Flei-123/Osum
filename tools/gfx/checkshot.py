...
PPM in P6), und dieses Programm rechnet sie nach:
  ...
  font       der Zeichensatz in kernel/drivers/gfx/font.fi gegen die Rust-Vorlage, aus
...
Der Zeichensatz wird IMMER aus `kernel/drivers/gfx/font.fi` gelesen, nie aus einer
...
def font_aus_firn(pfad):
    """Die zehn b"..."-Stuecke aus kernel/drivers/gfx/font.fi, in ihrer Reihenfolge."""
...
def cmd_font(a):
    ...
    if len(fi) != soll:
        print("kernel/drivers/gfx/font.fi hat %d Oktette, erwartet %d" % (len(fi), soll))
        return 1
    if len(a) < 2:
